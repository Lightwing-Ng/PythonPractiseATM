...
from core import FinacialFormat
import json, time
...
def acc_auth(account, password):
    ...
    if os.path.isfile(account_file):
        ...
        print("\033[31;1mAccount %s does not exist!\033[0m" % FinacialFormat.formatCardNum(account))
...
def acc_login(user_data, log_obj):
    ...
    while user_data['is_authenticated'] is not True and retry_count <= 3:
        ...
        if auth:  # not None means passed the authentication
            ...
            print("Welcome to use HSBC ATM!")
            return auth
        retry_count += 1

    else:
        log_obj.error(
            "Sorry, account [%s] failed with too many login attempts." % FinacialFormat.formatCardNum(account))
        exit()
